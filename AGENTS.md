...
- `validation/`: end-to-end validation manifests (k3d + MinIO + FIO), plus optional patches.
...
- `docker build -t ghcr.io/jrcichra/zerofs-csi-driver:local .`: build an image locally.
- `make build`: builds a linux binary and publishes a multi-arch image (uses `dbuildx`; see `Makefile`).
- `make run-validation`: creates a k3d cluster, deploys MinIO + driver, and runs the FIO workload.
- `make logs`: collects controller/node/FIO logs into `validation-logs.txt`.
...
- Kubernetes YAML: use stable resource names and keep the `zerofs` namespace consistent with existing manifests.

## Testing Guidelines

- There are currently no unit tests; use `go test ./...` as a compile sanity check and `go vet ./...` for basic static analysis.
- E2E validation is manifest-driven via `validation/` + `make run-validation`. Note: NBD support is required on the host.
...
- Avoid committing credentials; reference Kubernetes `Secret`s by name (see `validation/zerofs-config.yaml`).
